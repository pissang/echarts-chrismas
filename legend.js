
var childUrl = 'data:image/png;base64,';

var soundSource;

var isAndroid = navigator.userAgent.match(/(Android);?[\s\/]+([\d.]+)?/);
var isWeChart = (/micromessenger/i).test(navigator.userAgent);

var lowQuality = isAndroid && isWeChart;

var hasWebAudio = window.hasOwnProperty('AudioContext') || window.hasOwnProperty('webkitAudioContext');

legend.onApiReady = function() {
    // var span = legend.getComponentsByName('game-score')[0].surfaceElement.querySelector('span');
    // span && (span.innerHTML = 100);

    function updateAction(idx) {
        slideActions.forEach(function (actions, slideIdx) {
            if (slideIdx !== idx) {
                actions && actions.forEach(function (action) {
                    action.dispose();
                });
            }
            else {
                actions && actions.forEach(function (action) {
                    action.init();
                });
            }
        });
    }

    updateAction(legend.render.slider.slideIndex);

    legend.render.slider.on('slideChanged', function (idx) {
        currentIndex = idx;
        updateAction(idx);
    });
};

var fakeLoadingEffect = {
    dispose: function () {

        if (!this.chart) {
            return;
        }
        this.chart.dispose();
        this.chart = null;

        clearTimeout(this.timeout1);
        clearTimeout(this.timeout2);
    },

    init: function () {

        var dom = legend.getComponentsByName('fake-loading')[0].surfaceElement;
        var chart = this.chart = echarts.init(dom);
        var radius = 250;

        var option = {
            series: [{
                amplitude: 10,
                color: ['#eee', '#f3f3f3', '#fff'],
                type: 'liquidFill',
                radius: radius,
                data: [1, 1, 1],
                animationDuration: 0,
                animationDurationUpdate: 4000,
                label: {
                    normal: {
                        formatter: function() {
                            return '';
                        }
                    }
                },
                backgroundStyle: {
                    color: 'transparent',
                    borderWidth: 1,
                    borderColor: '#fff'
                },
                outline: {
                    borderDistance: 5,
                    itemStyle: {
                        borderColor: '#fff',
                        borderWidth: 5
                    }
                }
            }],
            graphic: {
                elements: [{
                    type: 'image',
                    style: {
                        image: 'data:image/png;base64,',
                        width: radius - 10,
                        height: radius - 10
                    },
                    left: 'center',
                    top: 'middle'
                }]
            }
        };
        chart.setOption(option);

        this.timeout1 = setTimeout(function() {
            chart.setOption({
                series: [{
                    data: [0.25, 0.25, 0.25]
                }]
            });
        }, 500);

        // this.timeout2 = setTimeout(function() {
        //     legend.render.gotoPage(1);
        // }, 7000);
    }
};

function createSnowDropEffect() {
    return {
        dispose: function () {

            if (!this.chart) {
                return;
            }
            this.chart.dispose();
            this.chart = null;

        },

        init: function () {
            if (this.chart) {
                return;
            }

            var dom = legend.getComponentsByName('snow-drop-' + legend.render.slider.slideIndex)[0].surfaceElement;
            var chart = this.chart = echarts.init(dom);

            var snowDrops = [];

            var canvas = document.createElement('canvas');
            canvas.width = canvas.height = 50;
            var circleCtx = canvas.getContext('2d');
            circleCtx.beginPath();
            circleCtx.arc(25, 25, 15, 0, Math.PI * 2);
            circleCtx.shadowColor = '#fff';
            circleCtx.shadowBlur = 15;
            circleCtx.fillStyle = '#fff';
            circleCtx.fill();

            function SnowDrop(size, speed, position, life) {
                this._el = new echarts.graphic.Image({
                    style: {
                        image: canvas,
                        x: -canvas.width / 2,
                        y: -canvas.height / 2
                    },
                    scale: [size / 20, size / 20],
                    position: position
                });

                this._opacity = size / 8;

                this._speed = speed;

                this.restLife = life;
            }

            SnowDrop.prototype.getEl = function () {
                return this._el;
            }

            SnowDrop.prototype.update = function (time) {
                this._el.position[0] += this._speed[0] * time;
                this._el.position[1] += this._speed[1] * time;

                this._el.style.opacity = this._opacity;

                this._el.dirty();

                this.restLife -= time;
                if (this.restLife < 0) {
                    return true;
                }
            };

            function spawnSnowDrop() {
                var size = Math.random() * 8 + 2;
                var speed = size * 20;

                var snowDrop = new SnowDrop(
                    size, [Math.random(), speed],
                    [zr.getWidth() * Math.random(), -Math.random() * 10],
                    4 + Math.sqrt(10 - size)
                );

                snowDrops.push(snowDrop);
                return snowDrop;
            }

            var zr = chart.getZr();

            zr.animation.on('frame', function (dTime) {
                dTime /= 1000;
                // spawn
                if (Math.random() < 0.1) {
                    for (var i = 0; i < 1; i++) {
                        var snowDrop = spawnSnowDrop();
                        zr.add(snowDrop.getEl());
                    }
                }
                snowDrops.forEach(function (snowDrop) {
                    snowDrop.update(dTime / 1000);
                });
                // Update
                snowDrops = snowDrops.filter(function (snowDrop) {
                    var needsRemove = snowDrop.update(dTime);
                    if (needsRemove) {
                        zr.remove(snowDrop.getEl());
                    }
                    return !needsRemove;
                });
            });
        }
    };
}

var snowDrop0 = createSnowDropEffect();
var snowDrop1 = createSnowDropEffect();

// 音频图
var fenceWave = {

    dispose: function () {
        if (!this.chart) {
            return;
        }
        this.chart.dispose();
        this.chart = null;

        clearInterval(this.interval);
    },

    init: function () {
        if (this.chart) {
            return;
        }

        var dom = legend.getComponentsByName('fence')[0].surfaceElement;
        var chart = this.chart = echarts.init(dom);

        var BIN_SIZE = 16;
        var FFT_ZERO = 128; // zero position in fft
        var FFT_MAX = 165;
        var FFT_MIN = 110;

        var audioData = [];
        var maxData = [];

        initChart();

        if (!soundSource) {

            var stopButton = legend.getComponentsByName('audio-on')[0].surfaceElement;
            var playButton = legend.getComponentsByName('audio-off')[0].surfaceElement;

            soundSource = new Audio();
            soundSource.src = fenceSong;
            soundSource.loop = true;
            soundSource.play();
            startUpdateChart(fenceFFT);

            var oldVolume;
            playButton.addEventListener('touchstart', function () {
                soundSource.play();
                // if (oldVolume) {
                //     soundSource.volume = oldVolume;
                // }
            });
            stopButton.addEventListener('touchstart', function () {
                soundSource.pause();
                // oldVolume = soundSource.volume;
                // soundSource.volume = 0;
            });
        }
        else {
            hasWebAudio && startUpdateChart(fenceFFT);
        }

        function startUpdateChart(fft) {
            fenceWave.interval = setInterval(function () {
                var time = soundSource.currentTime * 1000;
                var min = Infinity;
                var foundFrame;
                for (var i = 0; i < fft.length; i++) {
                    var frame = fft[i];
                    if (Math.abs(frame.t - time) < min) {
                        foundFrame = frame;
                        min = Math.abs(frame.t - time);
                    }
                }
                newData = [];
                for (var i = 0; i < foundFrame.d.length; ++i) {
                    maxData[i] = Math.max(Math.max(maxData[i] - 0.001, foundFrame.d[i]) - FFT_ZERO, 0);
                    newData.push(foundFrame.d[i]);
                }
                audioData = newData;

                chart.setOption({
                    series: [{
                        data: newData
                    }, {
                        data: maxData
                    }]
                });
            }, 250);
        }

        function initChart() {
            chart.setOption({
                series: [{
                    type: 'bar',
                    data: (function () {
                        var data = [];
                        maxData = [];
                        for (var i = 0; i < BIN_SIZE; ++i) {
                            data.push(FFT_ZERO);
                            maxData.push(0);
                        }
                        return data;
                    })(),
                    stack: 'wave',
                    barCategoryGap: '30%',
                    barGap: '-100%',
                    itemStyle: {
                        normal: {
                            barBorderRadius: 5,
                            color: '#fff'
                        }
                    },
                    barMinHeight: 20
                }, {
                    type: 'bar',
                    data: (function () {
                        var data = [];
                        for (var i = 0; i < BIN_SIZE; ++i) {
                            data.push(0);
                        }
                        return data;
                    })(),
                    stack: 'wave',
                    silent: true,
                    itemStyle: {
                        normal: {
                            color: 'transparent'
                        }
                    },
                    barMinHeight: 4
                }, {
                    type: 'bar',
                    data: (function () {
                        var data = [];
                        for (var i = 0; i < BIN_SIZE; ++i) {
                            data.push(0);
                        }
                        return data;
                    })(),
                    stack: 'wave',
                    silent: true,
                    itemStyle: {
                        normal: {
                            barBorderRadius: 5,
                            color: '#fff'
                        }
                    },
                    barMinHeight: 4
                }],
                xAxis: {
                    min: 0,
                    max: BIN_SIZE,
                    show: false,
                    data: (function () {
                        var data = [];
                        for (var i = 0; i < BIN_SIZE; ++i) {
                            data.push(i + '');
                        }
                        return data;
                    })()
                },
                yAxis: {
                    show: false,
                    min: FFT_MIN,
                    max: FFT_MAX
                },
                grid: {
                    left: 0,
                    right: 0,
                    top: 0,
                    bottom: 0
                }
            });
        }
    }
}

// 圣诞老人的驯鹿
var santaReindeer = {
    dispose: function () {
        if (this.chart) {
            this.chart.dispose();
            clearTimeout(this.timeout);
            this.chart = null;
        }
    },

    init: function () {
        if (this.chart) {
            return;
        }

        var dom = legend.getComponentsByName('santa-reindeer')[0].surfaceElement;
        var chart = this.chart = echarts.init(dom);

        this.timeout = setTimeout(function () {
            chart.setOption({
                grid: {
                    left: 5,
                    right: 0,
                    top: 0,
                    bottom: 0
                },
                xAxis: {
                    data: ['鲁道夫', '猛冲', '舞者', '乱舞', '雌狐', '彗星', '朱庇特', '唐纳', '闪电'],
                    axisLabel: {
                        interval: 0,
                        inside: true,
                        textStyle: {
                            color: 'rgba(255,255,255,0.8)',
                            fontSize: 12
                        }
                    },
                    splitLine: { show: false },
                    axisTick: { show: false },
                    axisLine: { show: false }
                },
                yAxis: {
                    show: false,
                    type: 'category',
                    data: ['驯鹿']
                },
                series: [{
                    type: 'scatter',
                    symbol: 'path://M-189.787,67.678c1.072,0.77-0.162,2.091-1.133,2.356c-0.809,0.184-0.858,0.419-0.912-1.61c-0.756-5.107-3.699-14.439-5.325-17.021c-0.503-0.736-0.738-1.273-0.738-1.273s-0.669,1.977-0.669,2.346c0,0.368-0.636,1.574-0.771,2.043s-3.279,9.141-3.948,10.146c-0.806,3.151,0.393,2.136-0.31,2.998c-0.091,0.776-1.762,1.158-2.553,0.815c-0.037-0.076,0.626-3.135,0.712-3.393c0.737-2.211,3.452-8.893,3.687-9.461c0.234-0.569,0.838-3.148,0.636-4.321c-0.201-1.173-0.603-2.814-1.842-3.552c-1.24-0.736-2.479-2.546-2.914-3.115c-0.436-0.568-0.738-2.713-2.412-2.311c-0.901,1.084-7.669,4.75-13.299,3.383c-2.345-0.569-4.188-1.072-4.858-1.072c-0.669,0-1.742-0.838-1.91,0.904c-0.166,1.742-0.736,4.154-0.803,5.159c0.711,2.651-0.865,14.542,1.148,18.295c0.959,1.786,0.509,1.983,0.383,1.995c-0.761,0.211-2.06,0.611-2.72,0.007c-0.208-0.19,0.162-1.132,0.212-1.565c0.685-2.406-1.993-16.574-2.105-19.166c-1.102,19.925,0.827,14.424,0.974,18.172c-0.586,0.369-1.503,0.747-2.542,0.512c-0.22-0.01-0.371-0.197-0.331-0.992c0.067-1.323,0.334-1.763,0.246-3.627c-0.235-5.023-1.443-12.921-1.33-13.932c0-1.439-0.235-3.584-0.435-4.422c-0.202-0.837-0.502-2.646-1.474-3.081c-5.729-1.687-10.635-21.806-11.088-26.466c-0.369-1.541-0.074-4.396-0.042-4.866c-0.368-0.436-1.466-0.241-2.404-1.097c-1.14-1.139-1.692-2.621-1.858-3.458c-0.083-0.493-0.36-0.126-1.458-1.099c-1.742-1.305-3.986-3.482-4.287-5.191c-0.206-0.83-0.462-3.273-0.33-3.174c0.099,0.031,0.782,3.25,1.15,3.299c0.786-0.419-0.018-2.387-0.018-3.826c0-1.44,0.018-2.479-0.971-3.803c-0.302-1.039,0.033-2.714,0.469-3.216c-0.268,1.388-0.26,2.369,0.36,3.333c0.712,0.947,1.247,1.089,2.353,0.184c1.221-0.836,2.266-1.81,2.411-2.102c0.034-0.07,0.023,0.258-0.379,0.906c-1.441,1.768-2.781,1.878-2.328,2.516c0.276,0.359,0.814,0.061,1.068-0.182c0.284-0.293,0.389,0.277-0.637,0.805c-1.048,0.447-1.507,0.335-1.44,2.279c0.036,1.062,0.039,2.916,0.727,4.478c0.567,1.294,1.671,2.353,3.087,2.877c0.927-0.044,1.388,0.569,2.106,0.974c0.475,0.268,1.169,0.301,1.65,0.448c1.207,0.368,1.575,0.904,1.575,0.904s0.569,0.503,0.067-0.437c-0.503-0.938-1.428-2.25-1.406-3.617c-0.035-1.406-0.134-2.277,0.401-0.302c0.662,3.159,2.396,3.702,2.278,5.243c0.479,0.4,0.888,0.041,1.399,0.092c0.678,0.041,1.014,0.469,1.716,0.168c0.461-0.303,0.896-0.41,1.475-0.311c0.746-0.436,0.469-1.816,1.239-2.479c1.071-1.406,1.624-3.551,1.574-4.221c0.065-1.512,0.21-1.324,0.436-0.368c0.427,2.103-0.771,4.204-1.172,4.79c-0.561,0.854-1.039,1.516-0.502,1.44c0.536-0.167,1.892-0.72,2.194-1.089c0.762-0.662,1.039-1.193,1.29-1.592c0.352-0.477,0.786-0.57,0.871,0.168c0.024,0.67-0.218,0.837-0.034,1.273c3.625-1.412,6.235-3.173,7.512-8.818c0.09-1.268-0.25-2.324-0.754-3.422c-0.645-1.412-1.642-3.612-2.467-4.98c-0.032-0.107,0.551,0.364,1.034,1.207c0.403,1.039,1.823,3.604,2.145,4.02c0.468,0.438,0.905,1.039,1.14,0.168c0.185-1.055,0.871-1.727,0.736-3.484c-0.016-1.59,0.093-1.557,0.427,0.62c-0.101,1.474-0.527,2.898-0.561,3.602c-0.201,0.637-0.537,3.351-1.072,4.255c-2.306,5.33-3.644,5.936-8.436,8.561c0.058,0.1,0.119,1.941-2.25,3.197c-0.77,0.426-0.872,1.14-0.872,2.58c-0.117,2.209,0.85,2.994,1.051,3.563c0.201,0.569,0.373,3.337,1.999,4.142c1.18,0.428,2.477,0.804,4.421,1.004c3.083-0.008,5.577,0.889,8.091,1.189c2.512,0.301,5.878-0.754,8.324-0.754c2.445,0,6.919-0.217,9.967-0.117c3.048,0.101,5.605,0.592,8.458,2.094c2.353,1.469,2.569,6.314,1.454,6.99c0.091-1.138-0.522-0.649-0.49-2.748c0.028,0.553-0.083,2.197-0.812,3.917l-1.408,4.22c0,0-1.724,4.036-0.051,6.381c1.676,2.346,2.715,3.25,3.049,3.585c0.336,0.335,0.67,1.173,0.771,3.249c0.1,2.078,0.703,4.455,0.938,5.562C-193.166,56.301-190.61,66.523-189.787,67.678z',
                    symbolSize: [35, 45],
                    symbolOffset: [-5, 0],
                    itemStyle: {
                        normal: {
                            color: 'rgba(255,255,255,0.6)'
                        }
                    },
                    data: [{
                        symbol: 'image://data:image/png;base64,',
                        value: 0
                    },0,0,0,0,0,0,0,0],

                    animationEasing: 'elasticOut',
                    animationDelay: function (idx) {
                        return idx * 100;
                    }
                }]
            });
        }, 2500);
    }
};

// 圣诞老人经过的国家
var santaCountryChart = {

    dispose: function () {},

    init: function () {
        var dom = legend.getComponentsByName('santa-country')[0].surfaceElement;
        var chart = echarts.init(dom);

        var result = {"data":[["Location","Whether celebrates"],["Afghanistan","0"],["Albania","1"],["Algeria","0"],["Andorra","1"],["Angola","1"],["Antigua and Barbuda","1"],["Argentina","1"],["Armenia","1"],["Australia","1"],["Austria","1"],["Azerbaijan","0.5"],["Bahamas","1"],["Bahrain","0"],["Bangladesh","1"],["Barbados","1"],["Belarus","1"],["Belgium","1"],["Belize","1"],["Benin","1"],["Bhutan","0"],["Bolivia","1"],["Bosnia and Herzegovina","1"],["Botswana","1"],["Brazil","1"],["Brunei Darussalam","1"],["Bulgaria","1"],["Burkina Faso","1"],["Burundi","1"],["Cambodia","0.5"],["Cameroon","1"],["Canada","1"],["Cape Verde","1"],["Central African Republic","1"],["Chad","1"],["Chile","1"],["China","0.5"],["Colombia","1"],["Comoros","0"],["Congo","1"],["Cook Islands","1"],["Costa Rica","1"],["Cote d'Ivoire","1"],["Croatia","1"],["Cuba","1"],["Cyprus","1"],["Czech Republic","1"],["Republic of the Congo","1"],["Democratic Republic of the Congo","1"],["Denmark","1"],["Djibouti","1"],["Dominica","1"],["Dominican Republic","1"],["Ecuador","1"],["Egypt","1"],["El Salvador","1"],["Equatorial Guinea","1"],["Eritrea","1"],["Estonia","1"],["Ethiopia","1"],["Fiji","1"],["Finland","1"],["France","1"],["Gabon","1"],["Gambia","1"],["Georgia","1"],["Germany","1"],["Ghana","1"],["Greece","1"],["Grenada","1"],["Guatemala","1"],["Guinea","1"],["Guinea Bissau","1"],["Guyana","1"],["Haiti","1"],["Honduras","1"],["Hungary","1"],["Iceland","1"],["India","1"],["Indonesia","1"],["Iran","0.5"],["Iraq","1"],["Ireland","1"],["Israel","0"],["Italy","1"],["Ivory Coast","1"],["Jamaica","1"],["Japan","0.5"],["Jordan","1"],["Kazakhstan","1"],["Kenya","1"],["Kiribati","1"],["Kuwait","0"],["Kyrgyzstan","1"],["Laos","0"],["Latvia","1"],["Lebanon","1"],["Lesotho","1"],["Liberia","1"],["Libya","0"],["Lithuania","1"],["Luxembourg","1"],["Madagascar","1"],["Malawi","1"],["Malaysia","1"],["Maldives","0"],["Mali","1"],["Malta","1"],["Marshall Islands","1"],["Mauritania","0"],["Mauritius","1"],["Mexico","1"],["Micronesia (Federated States of)","1"],["Monaco","1"],["Mongolia","0.5"],["Montenegro","1"],["Morocco","0.5"],["Mozambique","1"],["Myanmar","1"],["Namibia","1"],["Nauru","1"],["Nepal","1"],["Netherlands","1"],["New Zealand","1"],["Nicaragua","1"],["Niger","1"],["Nigeria","1"],["Niue","1"],["Norway","1"],["North Korea","0"],["Oman","0.5"],["Pakistan","0.5"],["Palau","1"],["Panama","1"],["Papua New Guinea","1"],["Paraguay","1"],["Peru","1"],["Philippines","1"],["Poland","1"],["Portugal","1"],["Qatar","0"],["Republic of Korea","1"],["Republic of Moldova","1"],["Romania","1"],["Rwanda","1"],["Saint Kitts and Nevis","1"],["Saint Lucia","1"],["Saint Vincent and the Grenadines","1"],["Samoa","1"],["San Marino","1"],["Sao Tome and Principe","1"],["Saudi Arabia","0"],["Senegal","1"],["Serbia","1"],["Seychelles","1"],["Sierra Leone","1"],["Singapore","1"],["Slovakia","1"],["Slovenia","1"],["Somaliland","1"],["Somalia","0"],["South Africa","1"],["South Korea","1"],["Spain","1"],["Sri Lanka","1"],["Sudan","1"],["South Sudan"," 1"],["Suriname","1"],["Swaziland","1"],["Sweden","1"],["Switzerland","1"],["Tajikistan","0"],["Thailand","0.5"],["Timor-Leste","1"],["Togo","1"],["Tonga","1"],["Trinidad and Tobago","1"],["Tunisia","0"],["Turkey","0.5"],["Turkmenistan","0"],["Tuvalu","1"],["Uganda","1"],["Ukraine","1"],["United Arab Emirates","0.5"],["United Kingdom","1"],["Uruguay","1"],["Uzbekistan","0"],["Vanuatu","1"],["Venezuela","1"],["Vietnam","0.5"],["Yemen","0"],["Zambia","1"],["Zimbabwe","1"],["United States","1"],["United Republic of Tanzania","1"],["Macedonia","1"],["Syria","1"],["Russia","1"],[""]],"errors":[],"meta":{"delimiter":",","linebreak":"\n","aborted":false,"truncated":false,"cursor":2394}};


        var nameMap = {
            'United States': 'United States of America'
        };
        chart.setOption({
            visualMap: {
                min: 0,
                max: 1,
                show: false,
            },
            series: [{
                type: 'map',
                map: 'world-simp',
                silent: true,
                top: 10,
                bottom: 10,
                data: result.data.map(function (item) {
                    return {
                        name: nameMap[item[0]] || item[0],
                        value: item[1]
                    };
                }),
                itemStyle: {
                    normal: {
                        borderColor: 'rgba(255,255,255,0.2)'
                    }
                }
            }],
            visualMap: {
                type:'piecewise',
                min: 0,
                max: 1,
                splitNumber: 3,
                left: 'left',
                top: 'bottom',
                seriesIndex:0,
                hoverLink: false,
                itemWidth: 15,
                textGap: 5,
                textStyle: {
                    color: '#fff'
                },
                pieces: [{
                    max: 0.1,
                    label: '不庆祝圣诞节',
                    color: '#eded7c'
                }, {
                    max: 0.6,
                    label: '假装过圣诞节',
                    color: '#f46f62'
                }, {
                    max: 1.1,
                    label: '国定假日',
                    color: '#e4503a'
                }]
            }
        });
    }
}


// 各个国家的气温
var countryTemperature = {
    dispose: function () {
        if (this.chart) {
            this.chart.dispose();
            this.chart = null;
        }
    },

    init: function () {
        if (this.chart) {
            return;
        }

        var dom = legend.getComponentsByName('country-temperature')[0].surfaceElement;
        var chart = this.chart = echarts.init(dom);
        var series = [];

        var data = [-28, -6, -3, 12, 22, 31, 40];
        var countries = ['俄罗斯', '中国', '美国', '英国', '墨西哥', '巴西', '澳大利亚'];

        var titles = [];

        for (var i = 0; i < 7; i++) {

            var x = 7 + i / 7 * 100 + '%';

            titles.push({
                text: countries[i],
                textAlign: 'center',
                left: x,
                bottom: 10,
                padding: 0,
                textStyle: {
                    color: '#fff',
                    fontSize: 12,
                    fontWeight: 'normal'
                }
            })

            series.push({

                animation: !lowQuality,
                waveAnimation: !lowQuality,

                color: ['rgb(253, 77, 73)'],
                center: [x, '50%'],
                radius: '63%',
                type: 'liquidFill',
                shape: 'path://M229.844,151.547v-166.75c0-11.92-9.662-21.582-21.58-21.582s-21.581,9.662-21.581,21.582v166.75c-9.088,6.654-14.993,17.397-14.993,29.524c0,20.2,16.374,36.575,36.574,36.575c20.199,0,36.574-16.375,36.574-36.575C244.838,168.944,238.932,158.201,229.844,151.547z',
                outline: {
                    show: false
                },
                amplitude: 1,
                waveLength: '20%',
                backgroundStyle: {
                    color: 'none',
                    borderColor: '#fff',
                    borderWidth: 5
                },
                data: [{
                    // -60 到 100 度
                    value: (data[i] - (-60)) / 160 + 0.1,
                    rawValue: data[i]
                }],
                itemStyle: {
                    normal: {
                        shadowBlur: 0
                    }
                },
                label: {
                    normal: {
                        position: 'insideBottom',
                        distance: 10,
                        formatter: function (item) {
                            return ' ' + item.data.rawValue + '°';
                        },
                        textStyle: {
                            color: 'rgb(253, 77, 73)',
                            fontSize: 16
                        }
                    }
                }
            })
        }

        chart.setOption({
            title: titles,
            series: series
        });
    }
};


// 每年究竟要有多少人过圣诞
var peopleHaveChrismasChart = {

    dispose: function () {
        if (this.chart) {
            this.chart.dispose();
            this.chart = null;
        }
    },

    init: function () {
        if (this.chart) {
            return;
        }

        var dom = legend.getComponentsByName('people-have-chrismas')[0].surfaceElement;
        var chart = this.chart = echarts.init(dom);
        var option = {
            series: [{
                type: 'liquidFill',
                animation: !lowQuality,
                waveAnimation: !lowQuality,

                data: [0.93, 0.88, 0.8],
                color: ['#49d088', '#38b470', '#2aaf66'],
                center: ['25%', '50%'],
                waveLength: '60%',
                amplitude: 8,
                radius: '70%',
                label: {
                    normal: {
                        formatter: function() {
                            return '世界\n\n93%人口'
                        },
                        textStyle: {
                            fontSize: 22,
                            color: '#44c078'
                        },
                        position: ['50%', '30%']
                    }
                },
                outline: {
                    itemStyle: {
                        borderColor: '#49d088',
                        borderWidth: 5
                    },
                    borderDistance: 0
                },
                itemStyle: {
                    normal: {
                        backgroundColor: '#fff'
                    }
                }
            }, {
                animation: !lowQuality,
                waveAnimation: !lowQuality,

                type: 'liquidFill',
                data: [0.66, 0.6, 0.5],
                color: ['#eb594b', '#e44d46', '#dc403f'],
                center: ['75%', '50%'],
                radius: '70%',
                amplitude: 8,
                label: {
                    normal: {
                        formatter: function() {
                            return '中国\n\n66%人口'
                        },
                        textStyle: {
                            fontSize: 22,
                            color: '#e35140'
                        },
                        position: ['50%', '30%']
                    }
                },
                outline: {
                    itemStyle: {
                        borderColor: '#e35140',
                        borderWidth: 5
                    },
                    borderDistance: 0
                },
                itemStyle: {
                    normal: {
                        backgroundColor: '#fff'
                    }
                }
            }]
        };

        chart.setOption(option);
    }
};
// 小孩的增长率
var childrenNeedsChart = {
    dispose: function () {
        if (this.chart) {
            this.chart.dispose();
            this.chart = null;
        }
    },

    init: function () {
        if (this.chart) {
            return;
        }

        var dom = legend.getComponentsByName('children-needs')[0].surfaceElement;
        var chart = this.chart = echarts.init(dom);

        var option = {
            grid: {
                top: 40,
                bottom: 40
            },
            xAxis: {
                type: 'category',
                boundaryGap: false,
                axisLine: {
                    show: true,
                    lineStyle: {
                        width: 2,
                        color: '#eb594b'
                    }
                },
                axisTick: {
                    show: false
                },
                axisLabel: {
                    textStyle: {
                        color: 'white',
                        fontSize: 16
                    }
                },
                splitLine: {
                    show: true,
                    lineStyle: {
                        width: 2,
                        color: '#eb594b',
                        type: 'dashed'
                    }
                },
                data: ['2011', '2012', '2013', '2014', '2015'],
            },
            yAxis: [{
                type: 'value',
                min: 18,
                show: false
            }, {
                type: 'value',
                min: 2.3,
                max: 2.5,
                show: false
            }],
            legend: {
                data: ['世界儿童人口', '中国儿童人口'],
                textStyle: {
                    color: '#fff'
                }
            },
            series: [{
                animation: !lowQuality,

                name: '世界儿童人口',
                type: 'line',
                symbol: 'circle',
                symbolSize: 10,
                lineStyle: {
                    normal: {
                        color: 'transparent',
                        width: 2,
                    },
                },
                itemStyle: {
                    normal: {
                        color: '#46d185'
                    }
                },
                areaStyle: {
                    normal: {
                        color: '#46d185'
                    }
                },
                label: {
                    normal: {
                        show: true,
                        position: 'top',
                        textStyle: {
                            fontSize: 16,
                            color: '#ddd'
                        }
                    },
                },
                data: [18.62, 18.74, 18.89, 19.04, 19.18]
            }, {
                animation: !lowQuality,

                name: '中国儿童人口',
                type: 'line',
                data: [2.311, 2.314, 2.328, 2.347, 2.363],
                itemStyle: {
                    normal: {
                        color: '#eb594b',
                        shadowColor: 'rgba(0, 0, 0, 0.5)',
                        shadowBlur: 20
                    }
                },
                yAxisIndex: 1,
                symbol: 'image://' + childUrl,
                symbolSize: 30,
                label: {
                    normal: {
                        show: true,
                        position: 'top',
                        textStyle: {
                            fontSize: 16,
                            color: 'white'
                        }
                    },
                },
                lineStyle: {
                    normal: {
                        width: 4,
                        shadowColor: 'rgba(0, 0, 0, 0.25)',
                        shadowBlur: 20
                    }
                }
            }]
        };

        chart.setOption(option);
    }
};

// 清单高度
var paperHeight = {

    dispose: function () {
        if (this.chart) {
            this.chart.dispose();
            this.chart = null;
        }
    },

    init: function () {

        if (this.chart) {
            return;
        }

        var dom = legend.getComponentsByName('paper-height')[0].surfaceElement;
        var chart = this.chart = echarts.init(dom);

        var paperUrl = 'data:image/png;base64,';
        var mountainUrl = 'data:image/png;base64,';

        chart.setOption({
            xAxis: {
                data: ['a', 'b'],
                axisTick: {show: false},
                axisLine: {show: false},
                axisLabel: {show: false}
            },
            yAxis: {
                splitLine: {show: false},
                axisTick: {show: false},
                axisLine: {show: false},
                axisLabel: {show: false}
            },
            grid: {
                top: 30,
                right: 10,
                bottom: 30,
                left: 10
            },
            markLine: {
                z: 20
            },
            series: [{
                name: 'all',
                type: 'pictorialBar',
                barCategoryGap: '40%',
                label: {
                    normal: {
                        show: true,
                        position: 'top',
                        formatter: '{c} km',
                        textStyle: {
                            fontSize: 16,
                            color: '#e54035'
                        }
                    }
                },
                data: [{
                    value: 6400,
                    symbol: 'image://' + paperUrl,
                    symbolRepeat: true,
                    symbolSize: ['80%', '20%'],
                    symbolMargin: '-38%'
                }, {
                    value: 8848,
                    symbol: 'image://' + mountainUrl,
                }],
                z: 10,
                markLine: {
                    symbol: ['none', 'none'],
                    label: {
                        normal: {show: false}
                    },
                    lineStyle: {
                        normal: {
                            color: '#e54035',
                            width: 2
                        }
                    },
                    data: [{
                        yAxis: 6700
                    }]
                },

                animationEasing: 'elasticOut'
            }, {
                name: 'shadow',
                type: 'pictorialBar',
                barGap: '-100%',
                symbol: 'circle',
                itemStyle: {
                    normal: {
                        color: '#13887c'
                    }
                },
                symbolSize: ['120%', 30],
                data: [1, 1],

                animationEasing: 'elasticOut'
            }]
        });
    }
};

// 节省了多少棵树
var savedTree = {
    dispose: function () {
        if (this.chart) {
            this.chart.dispose();
            this.chart = null;
        }
    },

    init: function () {

        if (this.chart) {
            return;
        }

        var dom = legend.getComponentsByName('saved-tree')[0].surfaceElement;
        var chart = this.chart = echarts.init(dom);

        var treeUrl = 'data:image/png;base64,';

        var lineCount = 4;
        var cateData = [];
        var series = [];
        var data = [];

        for (var i = 0; i < lineCount; i++) {
            cateData.push(i + '');
            data.push({
                value: 1,
                symbolOffset: (i % 2) ? ['50%', 0] : null
            });
        }

        chart.setOption({
            color: ['#e54035'],
            xAxis: {
                show: false
            },
            yAxis: {
                data: cateData,
                show: false
            },
            grid: {
                top: 10,
                bottom: 10,
                left: 10,
                right: 10
            },
            series: [{
                name: 'all',
                type: 'pictorialBar',
                symbol: 'image://' + treeUrl,
                symbolSize: [30, 50],
                symbolRepeat: true,
                data: data,

                animationEasing: 'elasticOut'
            }]
        });
    }
};


// 准备的圣诞礼物数量

var giftNumber = {
    dispose: function () {
        if (this.chart) {
            this.chart.dispose();
            this.chart = null;
        }
    },
    init: function () {
        if (this.chart) {
            return;
        }

        var dom = legend.getComponentsByName('gift-number')[0].surfaceElement;
        var chart = this.chart = echarts.init(dom);

        chart.setOption({
            color: ['#e54035'],
            xAxis: {
                show: false
            },
            yAxis: {
                data: ['a', 'b', 'c'],
                show: false
            },
            grid: {
                left: 10,
                right: 10,
                top: 10,
                bottom: 10
            },
            series: [{
                name: 'glyph',
                type: 'pictorialBar',
                symbol: 'image://' + childUrl,
                symbolRepeat: true,
                symbolSize: 40,
                data: [100, 100, 100],

                animationEasing: 'elasticOut'
            }, {
                name: 'glyph',
                type: 'pictorialBar',
                symbol: 'rect',
                barGap: '-100%',
                symbolSize: ['40%', 45],
                itemStyle: {
                    normal: {
                        color: '#e54035'
                    }
                },
                label: {
                    normal: {
                        show: true,
                        formatter: '16 亿',
                        textStyle: {
                            color: '#fff',
                            fontSize: 20,
                            fontFamily: 'Arial',
                            fontWeight: 'bold'
                        }
                    }
                },
                data: ['-', 100, '-'],
                z: 100,

                animationEasing: 'elasticOut'
            }]
        })
    }

}

// 小精灵个数
var elfinNumber = {

    dispose: function () {
        if (this.chart) {
            this.chart.dispose();
            this.chart = null;
        }
    },
    init: function () {
        if (this.chart) {
            return;
        }

        var dom = legend.getComponentsByName('elfin-number')[0].surfaceElement;
        var chart = this.chart = echarts.init(dom);

        var elfinUrl = 'data:image/png;base64,'
        chart.setOption({
            color: ['#e54035'],
            xAxis: {
                splitLine: {show: false},
                axisTick: {show: false},
                axisLine: {show: false},
                axisLabel: {show: false}
            },
            yAxis: {
                data: ['a', 'b', 'c'],
                axisTick: {show: false},
                axisLine: {show: false},
                axisLabel: {show: false}
            },
            grid: {
                left: 10,
                right: 10,
                top: 10,
                bottom: 10
            },
            series: [{
                name: 'glyph',
                type: 'pictorialBar',
                symbol: 'image://' + elfinUrl,
                symbolRepeat: true,
                symbolSize: 40,
                data: [100, 100, 100],

                animationEasing: 'elasticOut'
            }, {
                name: 'glyph',
                type: 'pictorialBar',
                symbol: 'rect',
                barGap: '-100%',
                symbolSize: ['40%', 45],
                itemStyle: {
                    normal: {
                        color: '#e54035'
                    }
                },
                label: {
                    normal: {
                        show: true,
                        formatter: '3000 个',
                        textStyle: {
                            color: '#fff',
                            fontSize: 20,
                            fontFamily: 'Arial',
                            fontWeight: 'bold'
                        }
                    }
                },
                data: ['-', 100, '-'],
                z: 100,

                animationEasing: 'elasticOut'
            }]
        });
    }
};

// 义务制造
var madeByYiWu = {
    dispose: function () {
        if (this.chart) {
            this.chart.dispose();
            this.chart = null;
        }
    },

    init: function () {
        if (this.chart) {
            return;
        }

        var dom = legend.getComponentsByName('made-by-yiwu')[0].surfaceElement;
        var chart = this.chart = echarts.init(dom);

        var giftGreen = 'data:image/png;base64,';
        var giftRed = 'data:image/png;base64,';
        chart.setOption({
            color: ['#e54035'],
            xAxis: {
                splitLine: {show: false},
                axisTick: {show: false},
                axisLine: {show: false},
                axisLabel: {show: false}
            },
            yAxis: {
                data: ['a', 'b'],
                axisTick: {show: false},
                axisLine: {show: false},
                axisLabel: {show: false}
            },
            grid: {
                left: 10,
                right: 10,
                top: 10,
                bottom: 10
            },
            series: [{
                name: 'all',
                type: 'pictorialBar',
                symbol: 'image://' + giftGreen,
                symbolRepeat: true,
                symbolSize: 30,
                symbolMargin: 10,
                data: [100, 100],

                animationEasing: 'elasticOut'
            }, {
                name: 'yiwu',
                type: 'pictorialBar',
                barGap: '-100%',
                symbol: 'image://' + giftRed,
                symbolRepeat: true,
                symbolSize: 30,
                symbolMargin: 10,
                data: [60, 60],
                z: 10,

                animationEasing: 'elasticOut'
            }]

        });
    }
};

// 热搜礼物
var hotSearch = {

    dispose: function () {
        if (this.chart) {
            this.chart.dispose();
            this.chart = null;
        }
    },

    init: function () {

        if (this.chart) {
            return;
        }

        var dom = legend.getComponentsByName('hot-search')[0].surfaceElement;
        var chart = this.chart = echarts.init(dom);

        var treeUrl = "data:image/png;base64,";

        var starUrl = "data:image/png;base64,";

        var presents = ['圣诞树', '贺卡', '圣诞礼盒', '围巾', '袜子', '苹果', '手链', '巧克力', '玫瑰', '香水', '乐高', '芭比', '项链', '抱枕', '变形金刚', '摆件', '魔方', '文具', '棒棒糖', '蓝牙耳帽', '超级飞侠', '暖手宝', '夜灯', '堆袜', '耳钉', '公仔', '手机壳', '八音盒','剃须刀','打火机','手表', '巴克球', '模型','音响', '蒙奇奇', '保温杯']

        var data = [];
        for (var i = 0; i < presents.length; ++i) {
            data.push({
                name: presents[i],
                value: (presents.length - i) * 20
            });
        }
        for (var i = 10; i < presents.length; ++i) {
            var cnt = Math.floor(Math.random() * 10);
            for (var j = 0; j < cnt; ++j) {
                data.push({
                    name: presents[i],
                    value: Math.random() * 10
                });
            }
        }

        var maskImage = new Image();
        maskImage.onload = function () {
            chart.setOption({
                tooltip: {
                    show: false
                },
                series: [{
                    type: 'wordCloud',
                    gridSize: 1,
                    sizeRange: [10, 35],
                    rotationRange: [0, 90],
                    rotationStep: 90,
                    maskImage: maskImage,
                    textStyle: {
                        normal: {
                            color: function(v) {
                                if (v.value > 600) {
                                    return 'rgb(0, 116, 111)';
                                }
                                else if (v.value > 200) {
                                    return 'rgb(0, 156, 147)';
                                }
                                else if (v.value > 9) {
                                    return 'rgb(70, 209, 133)';
                                }
                                else {
                                    return 'rgb(20, 186, 167)';
                                }
                            }
                        }
                    },
                    width: 600,
                    height: 500,
                    top: 40,
                    data: data
                }],
                graphic: {
                    elements: [{
                        type: 'image',
                        style: {
                            image: starUrl,
                            width: 40,
                            height: 40
                        },
                        left: 'center',
                        top: 40
                    }]
                }
            });
        };
        maskImage.src = treeUrl;

    }
};


// 礼物来源分布
var giftFrom = {

    dispose: function () {
        if (this.chart) {
            this.chart.dispose();
            this.chart = null;
        }
    },

    init: function () {

        if (this.chart) {
            return;
        }

        var dom = legend.getComponentsByName('gift-from')[0].surfaceElement;
        var chart = this.chart = echarts.init(dom);

        var giftImageUrl = "data:image/png;base64,";

        chart.setOption({
            graphic: {
                elements: [{
                    type: 'image',
                    style: {
                        image: giftImageUrl,
                        width: 30,
                        height: 30
                    },
                    left: 'center',
                    top: 'center'
                }]
            },
            series: [{
                type: 'pie',
                radius: [30, '55%'],
                center: ['50%', '50%'],
                roseType: 'radius',
                color: ['#f2c955', '#00a69d', '#46d185', '#ec5845'],
                data: [{
                        value: 7,
                        name: '其他'
                    }, {
                        value: 12,
                        name: '自己制作'
                    }, {
                        value: 38,
                        name: '线下购买'
                    }, {
                        value: 53,
                        name: '网上购买'
                    }
                ],
                label: {
                    normal: {
                        textStyle: {
                            fontSize: 14
                        },
                        formatter: function (param) {
                            return param.name + ':\n' + Math.round(param.percent) + '%';
                        }
                    }
                },
                labelLine: {
                    normal: {
                        smooth: true,
                        lineStyle: {
                            width: 2
                        }
                    }
                },
                itemStyle: {
                    normal: {
                        shadowBlur: 30,
                        shadowColor: 'rgba(0, 0, 0, 0.4)'
                    }
                },

                animationType: 'scale',
                animationEasing: 'elasticOut',
                animationDelay: function (idx) {
                    return Math.random() * 200;
                }
            }]
        });
    }
};


// 礼物来源分布
var giftPrice = {

    dispose: function () {
        if (this.chart) {
            this.chart.dispose();
            this.chart = null;
        }
    },

    init: function () {

        if (this.chart) {
            return;
        }

        var dom = legend.getComponentsByName('gift-price')[0].surfaceElement;
        var chart = this.chart = echarts.init(dom);

        var percentage = [0.58, 0.24, 0.16, 0.02];
        var radius = [0.6];
        for (var i = 1; i < percentage.length; ++i) {
            radius.push(radius[0] / Math.sqrt(percentage[0] / percentage[i]));
        }
        var colors = ['#ec5845', '#f28a7d', '#fbdcd8', '#fff'];
        var labels = ['≥500元：2%', '≥200元：16%', '≥100元：24%', '其他：58%'];
        var top = 0.5;

        var series = [];

        for (var i = 0; i < percentage.length; ++i) {
            series.push({
                type: 'pie',
                silent: true,
                name: labels[percentage.length - i - 1],
                radius: [0, radius[i] * 100 + '%'],
                label: {
                    normal: {
                        show: false,
                    }
                },
                color: [colors[i]],
                center: ['30%', (top + (-radius[i] + radius[0]) / 2) * 100 + '%'],
                data: [{
                    value: 1,
                    itemStyle: {
                        normal: {
                            color: colors[i],
                            shadowBlur: 30,
                            shadowColor: 'rgba(0, 0, 0, 0.25)'
                        }
                    }
                }],
                animationType: 'scale'
            });
        }
        chart.setOption({
            legend: {
                data: labels,
                top: 'center',
                right: 10,
                orient: 'vertical'
            },
            series: series
        });
    }
};

var vehicleSpeed = {

    dispose: function () {
        if (this.chart) {
            this.chart.dispose();
            this.chart = null;
        }
    },

    init: function () {

        if (this.chart) {
            return;
        }

        var dom = legend.getComponentsByName('vehicle-speed')[0].surfaceElement;
        var chart = this.chart = echarts.init(dom);


        var symbols = {
            reindeer: 'path://M-22.788,24.521c2.08-0.986,3.611-3.905,4.984-5.892 c-2.686,2.782-5.047,5.884-9.102,7.312c-0.992,0.005-0.25-2.016,0.34-2.362l1.852-0.41c0.564-0.218,0.785-0.842,0.902-1.347 c2.133-0.727,4.91-4.129,6.031-6.194c1.748-0.7,4.443-0.679,5.734-2.293c1.176-1.468,0.393-3.992,1.215-6.557 c0.24-0.754,0.574-1.581,1.008-2.293c-0.611,0.011-1.348-0.061-1.959-0.608c-1.391-1.245-0.785-2.086-1.297-3.313 c1.684,0.744,2.5,2.584,4.426,2.586C-8.46,3.012-8.255,2.901-8.04,2.824c6.031-1.952,15.182-0.165,19.498-3.937 c1.15-3.933-1.24-9.846-1.229-9.938c0.008-0.062-1.314-0.004-1.803-0.258c-1.119-0.771-6.531-3.75-0.17-3.33 c0.314-0.045,0.943,0.259,1.439,0.435c-0.289-1.694-0.92-0.144-3.311-1.946c0,0-1.1-0.855-1.764-1.98 c-0.836-1.09-2.01-2.825-2.992-4.031c-1.523-2.476,1.367,0.709,1.816,1.108c1.768,1.704,1.844,3.281,3.232,3.983 c0.195,0.203,1.453,0.164,0.926-0.468c-0.525-0.632-1.367-1.278-1.775-2.341c-0.293-0.703-1.311-2.326-1.566-2.711 c-0.256-0.384-0.959-1.718-1.67-2.351c-1.047-1.187-0.268-0.902,0.521-0.07c0.789,0.834,1.537,1.821,1.672,2.023 c0.135,0.203,1.584,2.521,1.725,2.387c0.102-0.259-0.035-0.428-0.158-0.852c-0.125-0.423-0.912-2.032-0.961-2.083 c-0.357-0.852-0.566-1.908-0.598-3.333c0.4-2.375,0.648-2.486,0.549-0.705c0.014,1.143,0.031,2.215,0.602,3.247 c0.807,1.496,1.764,4.064,1.836,4.474c0.561,3.176,2.904,1.749,2.281-0.126c-0.068-0.446-0.109-2.014-0.287-2.862 c-0.18-0.849-0.219-1.688-0.113-3.056c0.066-1.389,0.232-2.055,0.277-2.299c0.285-1.023,0.4-1.088,0.408,0.135 c-0.059,0.399-0.131,1.687-0.125,2.655c0.064,0.642-0.043,1.768,0.172,2.486c0.654,1.928-0.027,3.496,1,3.514 c1.805-0.424,2.428-1.218,2.428-2.346c-0.086-0.704-0.121-0.843-0.031-1.193c0.221-0.568,0.359-0.67,0.312-0.076 c-0.055,0.287,0.031,0.533,0.082,0.794c0.264,1.197,0.912,0.114,1.283-0.782c0.15-0.238,0.539-2.154,0.545-2.522 c-0.023-0.617,0.285-0.645,0.309,0.01c0.064,0.422-0.248,2.646-0.205,2.334c-0.338,1.24-1.105,3.402-3.379,4.712 c-0.389,0.12-1.186,1.286-3.328,2.178c0,0,1.729,0.321,3.156,0.246c1.102-0.19,3.707-0.027,4.654,0.269 c1.752,0.494,1.531-0.053,4.084,0.164c2.26-0.4,2.154,2.391-1.496,3.68c-2.549,1.405-3.107,1.475-2.293,2.984 c3.484,7.906,2.865,13.183,2.193,16.466c2.41,0.271,5.732-0.62,7.301,0.725c0.506,0.333,0.648,1.866-0.457,2.86 c-4.105,2.745-9.283,7.022-13.904,7.662c-0.977-0.194,0.156-2.025,0.803-2.247l1.898-0.03c0.596-0.101,0.936-0.669,1.152-1.139 c3.16-0.404,5.045-3.775,8.246-4.818c-4.035-0.718-9.588,3.981-12.162,1.051c-5.043,1.423-11.449,1.84-15.895,1.111 c-3.105,2.687-7.934,4.021-12.115,5.866c-3.271,3.511-5.188,8.086-9.967,10.414c-0.986,0.119-0.48-1.974,0.066-2.385l1.795-0.618 C-22.995,25.682-22.849,25.035-22.788,24.521z',
            thunder: 'path://M37.921,9.217 L13.079,36.729 L27.499,36.729 L15.253,56.783 L42.921,28.484 L28.921,28.484z',
            plane: 'path://M1.112,32.559l2.998,1.205l-2.882,2.268l-2.215-0.012L1.112,32.559z M37.803,23.96 c0.158-0.838,0.5-1.509,0.961-1.904c-0.096-0.037-0.205-0.071-0.344-0.071c-0.777-0.005-2.068-0.009-3.047-0.009 c-0.633,0-1.217,0.066-1.754,0.18l2.199,1.804H37.803z M39.738,23.036c-0.111,0-0.377,0.325-0.537,0.924h1.076 C40.115,23.361,39.854,23.036,39.738,23.036z M39.934,39.867c-0.166,0-0.674,0.705-0.674,1.986s0.506,1.986,0.674,1.986 s0.672-0.705,0.672-1.986S40.102,39.867,39.934,39.867z M38.963,38.889c-0.098-0.038-0.209-0.07-0.348-0.073 c-0.082,0-0.174,0-0.268-0.001l-7.127,4.671c0.879,0.821,2.42,1.417,4.348,1.417c0.979,0,2.27-0.006,3.047-0.01 c0.139,0,0.25-0.034,0.348-0.072c-0.646-0.555-1.07-1.643-1.07-2.967C37.891,40.529,38.316,39.441,38.963,38.889z M32.713,23.96 l-12.37-10.116l-4.693-0.004c0,0,4,8.222,4.827,10.121H32.713z M59.311,32.374c-0.248,2.104-5.305,3.172-8.018,3.172H39.629 l-25.325,16.61L9.607,52.16c0,0,6.687-8.479,7.95-10.207c1.17-1.6,3.019-3.699,3.027-6.407h-2.138 c-5.839,0-13.816-3.789-18.472-5.583c-2.818-1.085-2.396-4.04-0.031-4.04h0.039l-3.299-11.371h3.617c0,0,4.352,5.696,5.846,7.5 c2,2.416,4.503,3.678,8.228,3.87h30.727c2.17,0,4.311,0.417,6.252,1.046c3.49,1.175,5.863,2.7,7.199,4.027 C59.145,31.584,59.352,32.025,59.311,32.374z M22.069,30.408c0-0.815-0.661-1.475-1.469-1.475c-0.812,0-1.471,0.66-1.471,1.475 s0.658,1.475,1.471,1.475C21.408,31.883,22.069,31.224,22.069,30.408z M27.06,30.408c0-0.815-0.656-1.478-1.466-1.478 c-0.812,0-1.471,0.662-1.471,1.478s0.658,1.477,1.471,1.477C26.404,31.885,27.06,31.224,27.06,30.408z M32.055,30.408 c0-0.815-0.66-1.475-1.469-1.475c-0.808,0-1.466,0.66-1.466,1.475s0.658,1.475,1.466,1.475 C31.398,31.883,32.055,31.224,32.055,30.408z M37.049,30.408c0-0.815-0.658-1.478-1.467-1.478c-0.812,0-1.469,0.662-1.469,1.478 s0.656,1.477,1.469,1.477C36.389,31.885,37.049,31.224,37.049,30.408z M42.039,30.408c0-0.815-0.656-1.478-1.465-1.478 c-0.811,0-1.469,0.662-1.469,1.478s0.658,1.477,1.469,1.477C41.383,31.885,42.039,31.224,42.039,30.408z M55.479,30.565 c-0.701-0.436-1.568-0.896-2.627-1.347c-0.613,0.289-1.551,0.476-2.73,0.476c-1.527,0-1.639,2.263,0.164,2.316 C52.389,32.074,54.627,31.373,55.479,30.565z',
            train: 'path://M67.335,33.596L67.335,33.596c-0.002-1.39-1.153-3.183-3.328-4.218h-9.096v-2.07h5.371 c-4.939-2.07-11.199-4.141-14.89-4.141H19.72v12.421v5.176h38.373c4.033,0,8.457-1.035,9.142-5.176h-0.027 c0.076-0.367,0.129-0.751,0.129-1.165L67.335,33.596L67.335,33.596z M27.999,30.413h-3.105v-4.141h3.105V30.413z M35.245,30.413 h-3.104v-4.141h3.104V30.413z M42.491,30.413h-3.104v-4.141h3.104V30.413z M49.736,30.413h-3.104v-4.141h3.104V30.413z  M14.544,40.764c1.143,0,2.07-0.927,2.07-2.07V35.59V25.237c0-1.145-0.928-2.07-2.07-2.07H-9.265c-1.143,0-2.068,0.926-2.068,2.07 v10.351v3.105c0,1.144,0.926,2.07,2.068,2.07H14.544L14.544,40.764z M8.333,26.272h3.105v4.141H8.333V26.272z M1.087,26.272h3.105 v4.141H1.087V26.272z M-6.159,26.272h3.105v4.141h-3.105V26.272z M-9.265,41.798h69.352v1.035H-9.265V41.798z',
            ship: 'path://M16.678,17.086h9.854l-2.703,5.912c5.596,2.428,11.155,5.575,16.711,8.607c3.387,1.847,6.967,3.75,10.541,5.375 v-6.16l-4.197-2.763v-5.318L33.064,12.197h-11.48L20.43,15.24h-4.533l-1.266,3.286l0.781,0.345L16.678,17.086z M49.6,31.84 l0.047,1.273L27.438,20.998l0.799-1.734L49.6,31.84z M33.031,15.1l12.889,8.82l0.027,0.769L32.551,16.1L33.031,15.1z M22.377,14.045 h9.846l-1.539,3.365l-2.287-1.498h1.371l0.721-1.352h-2.023l-0.553,1.037l-0.541-0.357h-0.34l0.359-0.684h-2.025l-0.361,0.684 h-3.473L22.377,14.045z M23.695,20.678l-0.004,0.004h0.004V20.678z M24.828,18.199h-2.031l-0.719,1.358h2.029L24.828,18.199z  M40.385,34.227c-12.85-7.009-25.729-14.667-38.971-12.527c1.26,8.809,9.08,16.201,8.213,24.328 c-0.553,4.062-3.111,0.828-3.303,7.137c15.799,0,32.379,0,48.166,0l0.066-4.195l1.477-7.23 C50.842,39.812,45.393,36.961,40.385,34.227z M13.99,35.954c-1.213,0-2.195-1.353-2.195-3.035c0-1.665,0.98-3.017,2.195-3.017 c1.219,0,2.195,1.352,2.195,3.017C16.186,34.604,15.213,35.954,13.99,35.954z M23.691,20.682h-2.02l-0.588,1.351h2.023 L23.691,20.682z M19.697,18.199l-0.721,1.358h2.025l0.727-1.358H19.697z',
            car: 'path://M49.592,40.883c-0.053,0.354-0.139,0.697-0.268,0.963c-0.232,0.475-0.455,0.519-1.334,0.475 c-1.135-0.053-2.764,0-4.484,0.068c0,0.476,0.018,0.697,0.018,0.697c0.111,1.299,0.697,1.342,0.931,1.342h3.7 c0.326,0,0.628,0,0.861-0.154c0.301-0.196,0.43-0.772,0.543-1.78c0.017-0.146,0.025-0.336,0.033-0.56v-0.01 c0-0.068,0.008-0.154,0.008-0.25V41.58l0,0C49.6,41.348,49.6,41.09,49.592,40.883L49.592,40.883z M6.057,40.883 c0.053,0.354,0.137,0.697,0.268,0.963c0.23,0.475,0.455,0.519,1.334,0.475c1.137-0.053,2.762,0,4.484,0.068 c0,0.476-0.018,0.697-0.018,0.697c-0.111,1.299-0.697,1.342-0.93,1.342h-3.7c-0.328,0-0.602,0-0.861-0.154 c-0.309-0.18-0.43-0.772-0.541-1.78c-0.018-0.146-0.027-0.336-0.035-0.56v-0.01c0-0.068-0.008-0.154-0.008-0.25V41.58l0,0 C6.057,41.348,6.057,41.09,6.057,40.883L6.057,40.883z M49.867,32.766c0-2.642-0.344-5.224-0.482-5.507 c-0.104-0.207-0.766-0.749-2.271-1.773c-1.522-1.042-1.487-0.887-1.766-1.566c0.25-0.078,0.492-0.224,0.639-0.241 c0.326-0.034,0.345,0.274,1.023,0.274c0.68,0,2.152-0.18,2.453-0.48c0.301-0.303,0.396-0.405,0.396-0.672 c0-0.268-0.156-0.818-0.447-1.146c-0.293-0.327-1.541-0.49-2.273-0.585c-0.729-0.095-0.834,0-1.022,0.121 c-0.304,0.189-0.32,1.919-0.32,1.919l-0.713,0.018c-0.465-1.146-1.11-3.452-2.117-5.269c-1.103-1.979-2.256-2.599-2.737-2.754 c-0.474-0.146-0.904-0.249-4.131-0.576c-3.298-0.344-5.922-0.388-8.262-0.388c-2.342,0-4.967,0.052-8.264,0.388 c-3.229,0.336-3.66,0.43-4.133,0.576s-1.633,0.775-2.736,2.754c-1.006,1.816-1.652,4.123-2.117,5.269L9.87,23.109 c0,0-0.008-1.729-0.318-1.919c-0.189-0.121-0.293-0.225-1.023-0.121c-0.732,0.104-1.98,0.258-2.273,0.585 c-0.293,0.327-0.447,0.878-0.447,1.146c0,0.267,0.094,0.379,0.396,0.672c0.301,0.301,1.773,0.48,2.453,0.48 c0.68,0,0.697-0.309,1.023-0.274c0.146,0.018,0.396,0.163,0.637,0.241c-0.283,0.68-0.24,0.524-1.764,1.566 c-1.506,1.033-2.178,1.566-2.271,1.773c-0.139,0.283-0.482,2.865-0.482,5.508s0.189,5.02,0.189,5.86c0,0.354,0,0.976,0.076,1.565 c0.053,0.354,0.129,0.697,0.268,0.966c0.232,0.473,0.447,0.516,1.334,0.473c1.137-0.051,2.779,0,4.477,0.07 c1.135,0.043,2.297,0.086,3.33,0.11c2.582,0.051,1.826-0.379,2.928-0.36c1.102,0.016,5.447,0.196,9.424,0.196 c3.976,0,8.332-0.182,9.423-0.196c1.102-0.019,0.346,0.411,2.926,0.36c1.033-0.018,2.195-0.067,3.332-0.11 c1.695-0.062,3.348-0.121,4.477-0.07c0.886,0.043,1.103,0,1.332-0.473c0.132-0.269,0.218-0.611,0.269-0.966 c0.086-0.592,0.078-1.213,0.078-1.565C49.678,37.793,49.867,35.408,49.867,32.766L49.867,32.766z M13.219,19.735 c0.412-0.964,1.652-2.9,2.256-3.244c0.145-0.087,1.426-0.491,4.637-0.706c2.953-0.198,6.217-0.276,7.73-0.276 c1.513,0,4.777,0.078,7.729,0.276c3.201,0.215,4.502,0.611,4.639,0.706c0.775,0.533,1.842,2.28,2.256,3.244 c0.412,0.965,0.965,2.858,0.861,3.116c-0.104,0.258,0.104,0.388-1.291,0.275c-1.387-0.103-10.088-0.216-14.185-0.216 c-4.088,0-12.789,0.113-14.184,0.216c-1.395,0.104-1.188-0.018-1.291-0.275C12.254,22.593,12.805,20.708,13.219,19.735 L13.219,19.735z M16.385,30.511c-0.619,0.155-0.988,0.491-1.764,0.482c-0.775,0-2.867-0.353-3.314-0.371 c-0.447-0.017-0.842,0.302-1.076,0.362c-0.23,0.06-0.688-0.104-1.377-0.318c-0.688-0.216-1.092-0.155-1.316-1.094 c-0.232-0.93,0-2.264,0-2.264c1.488-0.068,2.928,0.069,5.621,0.826c2.693,0.758,4.191,2.213,4.191,2.213 S17.004,30.357,16.385,30.511L16.385,30.511z M36.629,37.293c-1.23,0.164-6.386,0.207-8.794,0.207c-2.412,0-7.566-0.051-8.799-0.207 c-1.256-0.164-2.891-1.67-1.764-2.865c1.523-1.627,1.24-1.576,4.701-2.023C24.967,32.018,27.239,32,27.834,32 c0.584,0,2.865,0.025,5.859,0.404c3.461,0.447,3.178,0.396,4.699,2.022C39.521,35.623,37.887,37.129,36.629,37.293L36.629,37.293z  M48.129,29.582c-0.232,0.93-0.629,0.878-1.318,1.093c-0.688,0.216-1.145,0.371-1.377,0.319c-0.231-0.053-0.627-0.371-1.074-0.361 c-0.448,0.018-2.539,0.37-3.313,0.37c-0.772,0-1.146-0.328-1.764-0.481c-0.621-0.154-0.966-0.154-0.966-0.154 s1.49-1.464,4.191-2.213c2.693-0.758,4.131-0.895,5.621-0.826C48.129,27.309,48.361,28.643,48.129,29.582L48.129,29.582z',
            run: 'path://M13.676,32.955c0.919-0.031,1.843-0.008,2.767-0.008v0.007c0.827,0,1.659,0.041,2.486-0.019 c0.417-0.028,1.118,0.325,1.14-0.545c0.014-0.637-0.156-1.279-0.873-1.367c-1.919-0.241-3.858-0.233-5.774,0.019 c-0.465,0.062-0.998,0.442-0.832,1.069C12.715,32.602,13.045,32.977,13.676,32.955z M14.108,29.013 c1.47-0.007,2.96-0.122,4.414,0.035c1.792,0.192,3.1-0.412,4.273-2.105c-3.044,0-5.882,0.014-8.719-0.01 c-0.768-0.005-1.495,0.118-1.461,1C12.642,28.731,13.329,29.014,14.108,29.013z M23.678,36.593c-0.666-0.69-1.258-1.497-2.483-1.448 c-2.341,0.095-4.689,0.051-7.035,0.012c-0.834-0.014-1.599,0.177-1.569,1.066c0.031,0.854,0.812,1.062,1.636,1.043 c1.425-0.033,2.852-0.01,4.278-0.01v-0.01c1.562,0,3.126,0.008,4.691-0.005C23.614,37.239,24.233,37.174,23.678,36.593z  M32.234,42.292h-0.002c-1.075,0.793-2.589,0.345-3.821,1.048c-0.359,0.193-0.663,0.465-0.899,0.799 c-1.068,1.623-2.052,3.301-3.117,4.928c-0.625,0.961-0.386,1.805,0.409,2.395c0.844,0.628,1.874,0.617,2.548-0.299 c1.912-2.573,3.761-5.197,5.621-7.814C33.484,42.619,33.032,42.387,32.234,42.292z M43.527,28.401 c-0.688-1.575-2.012-0.831-3.121-0.895c-1.047-0.058-2.119,1.128-3.002,0.345c-0.768-0.677-1.213-1.804-1.562-2.813 c-0.45-1.305-1.495-2.225-2.329-3.583c2.953,1.139,4.729,0.077,5.592-1.322c0.99-1.61,0.718-3.725-0.627-4.967 c-1.362-1.255-3.414-1.445-4.927-0.452c-1.933,1.268-2.206,2.893-0.899,6.11c-2.098-0.659-3.835-1.654-5.682-2.383 c-0.735-0.291-1.437-1.017-2.293-0.666c-2.263,0.927-4.522,1.885-6.723,2.95c-1.357,0.658-1.649,1.593-1.076,2.638 c0.462,0.851,1.643,1.126,2.806,0.617c0.993-0.433,1.994-0.857,2.951-1.374c1.599-0.86,3.044-0.873,4.604,0.214 c1.017,0.707,0.873,1.137,0.123,1.849c-1.701,1.615-3.516,3.12-4.933,5.006c-1.042,1.388-0.993,2.817,0.255,4.011 c1.538,1.471,3.148,2.869,4.708,4.315c0.485,0.444,0.907,0.896-0.227,1.104c-1.523,0.285-3.021,0.694-4.538,1.006 c-1.109,0.225-2.02,1.259-1.83,2.16c0.223,1.07,1.548,1.756,2.687,1.487c3.003-0.712,6.008-1.413,9.032-2.044 c1.549-0.324,2.273-1.869,1.344-3.115c-0.868-1.156-1.801-2.267-2.639-3.445c-1.964-2.762-1.95-2.771,0.528-5.189 c1.394-1.357,1.379-1.351,2.437,0.417c0.461,0.769,0.854,1.703,1.99,1.613c2.238-0.181,4.407-0.755,6.564-1.331 C43.557,30.447,43.88,29.206,43.527,28.401z',
            walk: 'path://M29.902,23.275c1.86,0,3.368-1.506,3.368-3.365c0-1.859-1.508-3.365-3.368-3.365 c-1.857,0-3.365,1.506-3.365,3.365C26.537,21.769,28.045,23.275,29.902,23.275z M36.867,30.74c-1.666-0.467-3.799-1.6-4.732-4.199 c-0.932-2.6-3.131-2.998-4.797-2.998s-7.098,3.894-7.098,3.894c-1.133,1.001-2.1,6.502-0.967,6.769 c1.133,0.269,1.266-1.533,1.934-3.599c0.666-2.065,3.797-3.466,3.797-3.466s0.201,2.467-0.398,3.866 c-0.599,1.399-1.133,2.866-1.467,6.198s-1.6,3.665-3.799,6.266c-2.199,2.598-0.6,3.797,0.398,3.664 c1.002-0.133,5.865-5.598,6.398-6.998c0.533-1.397,0.668-3.732,0.668-3.732s0,0,2.199,1.867c2.199,1.865,2.332,4.6,2.998,7.73 s2.332,0.934,2.332-0.467c0-1.401,0.269-5.465-1-7.064c-1.265-1.6-3.73-3.465-3.73-5.265s1.199-3.732,1.199-3.732 c0.332,1.667,3.335,3.065,5.599,3.399C38.668,33.206,38.533,31.207,36.867,30.74z'
        };
        var rawData = [
            {name: '驯鹿', velocity: 123, symbol: 'reindeer'},
            {name: '光速', velocity: 60, symbol: 'thunder'},
            {name: '飞机', velocity: 25, symbol: 'plane'},
            {name: '高铁', velocity: 18, symbol: 'train'},
            {name: '轮船', velocity: 12, symbol: 'ship'},
            {name: '汽车', velocity: 9, symbol: 'car'},
            {name: '跑步', velocity: 2, symbol: 'run'},
            {name: '步行', velocity: 1, symbol: 'walk'}
        ];

        chart.setOption({
            xAxis: {
                data: rawData.map(function (item) {
                    return item.name;
                }),
                axisTick: {show: false},
                axisLine: {show: false},
                axisLabel: {
                    textStyle: {
                        color: '#e54035'
                    }
                }
            },
            grid: {
                left: 10,
                top: 10,
                right: 10,
                bottom: 50
            },
            yAxis: {
                splitLine: {show: false},
                axisTick: {show: false},
                axisLine: {show: false},
                axisLabel: {show: false}
            },
            color: ['#e54035'],
            series: [{
                name: 'hill',
                type: 'pictorialBar',
                barCategoryGap: '30%',
                symbol: 'path://M0,10 L10,10 L5,0 L0,10 z',
                data: rawData.map(function (item) {
                    return item.velocity;
                })
            }, {
                name: 'glyph',
                type: 'pictorialBar',
                barGap: '-100%',
                barCategoryGap: '30%',
                symbolPosition: 'end',
                symbolSize: [25, 25],
                symbolOffset: [0, -35],
                data: rawData.map(function (item) {
                    return {
                        value: item.velocity,
                        symbol: symbols[item.symbol]
                    };
                })
            }]
        });
    }
};

game.gameOver = function () {

    var childrenCnt;
    if (window.game.score === 0) {
        childrenCnt = 0;
    }
    else {
        childrenCnt = Math.floor((window.game.score + Math.random()) * 100);
    }

    var span = legend.getComponentsByName('game-score')[0].surfaceElement;
    span.innerHTML = childrenCnt;
    span.style.fontSize = '26px';
    span.style.color = 'rgb(237,79,72)';
    span.style.textAlign = 'center';

    legend.render.gotoPage(15);

};

var gamePage = {
    init: function () {

        game.init(legend.getComponentsByName('game-stage')[0].surfaceElement);
    },
    dispose: function () {
        game.dispose();
    }
};

var slideActions = [
    // 1
    [snowDrop0, fakeLoadingEffect],
    // 2
    [snowDrop1, fenceWave],
    // 3
    [santaReindeer],
    // 4
    [santaCountryChart, countryTemperature],
    // 5
    [peopleHaveChrismasChart, childrenNeedsChart],
    // 6
    [paperHeight, savedTree],
    // 7
    [giftNumber, elfinNumber, madeByYiWu],
    // 8
    [hotSearch],
    // 9
    [giftFrom, giftPrice],
    // 10
    [vehicleSpeed],
    // 11
    [],
    // 12
    [gamePage]
];